...
import { database, sendValueToFirebase4 } from '../firebase/firbase';
...
export default function Conhouse2 ({ route, navigation }){
...
useEffect(() => {
	const fetchData = async () => {
	  try {
		...
		const snapshotMode = await get(child(dbRef, `Node2/Zone2${username}`));
		...
		if (snapshotValve.exists()) {
		  ...
		  if (valveData["Valve"] === "Zone_2_OFF") {
			setStatus("Zone_2_OFF");
		  } else if (valveData["Valve"] === "Zone_2_ON") {
			setStatus("Zone_2_ON");
		  }
		}
	  } catch (error) {
    ...
    return (
    	...
    	>
		   ...
		   <TouchableHighlight style={[styles.items,]}
			   ...
			   onPress={
				   () => navigation.navigate('Setmodehouse2')}
		   >
		   ...
		   <TouchableHighlight
				...
				onPress={async () => {
					...
					await sendValueToFirebase4(value);
