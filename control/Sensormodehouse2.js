...
export default function Sensormodehouse2({ route, navigation }){
  ...
  setTest3(newEnabled);
    ...
    useEffect(() => {
      const intervalId = setInterval(() => {
        get(child(dbRef, `Node2/Zone2/ModeSensor${username}`))
          .then((snapshot) => {
            if (snapshot.exists()) {
    ...
    function updateData() {
        const path = `Node2/Zone2/ModeSensor${String(username)}`;
